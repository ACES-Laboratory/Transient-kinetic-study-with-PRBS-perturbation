function error=objlqsp(kfit,diffz,Di,QT,mc,Hb,CT,T,P,QS,void,dp,n,tspan,Rid,COinput,O2input,CO2input,yCO2,yCO,yO2,x05step,Cexp,Kr)
...
WO2=Cm/max(Cexp(:,2));
WCO2=Cm/max(Cexp(:,3));
Cfit=[WCO*pchip(ts,CCO(:,n),tspan(2:end));WO2*pchip(ts,CO2(:,n),tspan(2:end));WCO2*pchip(ts,CCO2(:,n),tspan(2:end));WCO*diff(pchip(ts,CCO(:,n),tspan(2:end)));WO2*diff(pchip(ts,CO2(:,n),tspan(2:end)));WCO2*diff(pchip(ts,CCO2(:,n),tspan(2:end)))];
