...
O2ins=zeros(length(deltat)*2+2,1);
...
O2in=pchip(tRs,O2ins,tR);
tR=[0,tR+steady];
O2in=[0,O2in];
CO2in=zeros(1,nsample+1);
COin=ones(1,nsample+1);
%space & time
n=101;
diffz=Hb/(n-1);
xs=linspace(0,Hb,n);
...
CO2input=yCO2*CT*CO2in*QT./(QN+QS*O2in);
O2input=O2in*yO2*CT*QT./(QN+QS*O2in);
COinput=COin*yCO*CT*QT./(QN+QS*O2in);
%% 
%kinetics
...
[ts,Cpro]=ode23tb(@odefun5step,tspan,x05step,options,diffz,Di,QT,mc,Hb,CT,T,P,QS,void,dp,n,tspan,Rid,COinput,O2input,CO2input,k5step,Ct,yCO2,yCO,yO2);
...
%plot(ts(2:end),CCO2f(2:end,n)*1.2,'LineWidth',1.5)
...
%plot(ts,CCO2(:,1),'LineWidth',1.5)
ylim([-0.1 round(1.1*max([CCOf;CO2f;CCO2f],[],'all'),1)])
%legend('CO inlet','O_2 inlet','CO_2 inlet')
