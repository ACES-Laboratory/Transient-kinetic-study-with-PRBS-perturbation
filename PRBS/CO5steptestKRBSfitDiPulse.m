...
figure()
plot(ts,CCO(:,1),'LineWidth',3.5,'Color',[0.9290 0.6940 0.1250])
hold on
plot(ts,CO2(:,1),'LineWidth',3.5,'Color',[0.4940 0.1840 0.5560])
%plot(ts,CCO2(:,1),'LineWidth',1.5)
ylim([0 3])
%legend('CO inlet','O_2 inlet','CO_2 inlet')
lgd=legend('A inlet','B_2 inlet');
xlabel('t/s','FontSize',25)
ylabel('C/(mol/m^3)','FontSize',25)
lgd.FontSize=25;
ax = gca;
ax.FontSize=25;
hold off
figure()
plot(ts,CCO(:,n),'LineWidth',3.5,'Color',[0.9290 0.6940 0.1250])
hold on
plot(ts,CO2(:,n),'LineWidth',3.5,'Color',[0.4940 0.1840 0.5560])
plot(ts,CCO2(:,n),'LineWidth',3.5,'Color',[0.8500 0.3250 0.0980])
lgd=legend('A outlet','B_2 outlet','AB outlet');
ylim([0 3.5])
xlabel('t/s','FontSize',25)
ylabel('C/(mol/m^3)','FontSize',25)
lgd.FontSize=25;
...
zlabel('\theta_{AB}')
figure()
surf(ts,xs,theCO2a'+theOa'+theO2a'+theCOa')
shading interp
xlabel('t/s')
ylabel('x/m')
zlabel('\theta_{T}')
...
%
figure()
plot(ts,Cexp(:,1),'LineWidth',1.5)
hold on
plot(ts,Cexp(:,2),'LineWidth',1.5)
plot(ts,Cexp(:,3),'LineWidth',1.5)
lgd=legend('Synthetic A outlet','Synthetic B_2 outlet','Synthetic AB outlet');
lgd.FontSize=15;
...
hold off
%
optionsop=optimset('Display','iter','Algorithm','trust-region-reflective','UseParallel',true,'TolX',1e-6,'MaxFunEvals',100000,'MaxIter',100000,'TolFun',1e-8,'FinDiffRelStep',0.0001);
ko=[k5step(1:7),k5step(9:10),Ct];
k0=0.1*ones(1,10);
lb=zeros(1,10);
ub=ones(1,9)*inf;
ub=[ub,10];
[kfit,resnorm,residual,exitflag,output,lambda,J] = lsqnonlin(@(k)objlqdp(k,diffz,Di,QT,mc,Hb,CT,T,P,QS,void,dp,n,tspan,Rid,COinput,O2input,CO2input,yCO2,yCO,yO2,x05step,Cexp,Kr),k0,lb,ub,optionsop);
...
relerror=error./ko
%%
[ts2,Cprof]=ode23tb(@(t,C)odefun5step(t,C,diffz,Di,QT,mc,Hb,CT,T,P,QS,void,dp,n,tspan,Rid,COinput,O2input,CO2input,k5stepf,Ctf,yCO2,yCO,yO2),tspan,x05step,options);
...
plot(ts,Cexp(:,3),'LineWidth',1.5)
hold on
plot(ts2,CCO2f(:,n),'LineWidth',1.5)
lgd=legend('Synthetic AB outlet','Fitted AB outlet');
ylim([0 round(1.4*max([CCO2 CCO2f],[],'all'),2)])
xlabel('t/s')
ylabel('C/(mol/m^3)')
lgd.FontSize=15;
ax = gca;
...
zlabel('\theta_{B}')
figure()
surf(ts2,xs,theCO2af')
shading interp
xlabel('t/s')
ylabel('x/m')
zlabel('\theta_{AB}')
figure()
surf(ts2,xs,theCO2af'+theOaf'+theO2af'+theCOaf')
shading interp
xlabel('t/s')
ylabel('x/m')
zlabel('\theta_{T}')
figure()
plot(ts2,CCOf(:,1),'LineWidth',1.5)
hold on
plot(ts2,CO2f(:,1),'LineWidth',1.5)
ylim([0 round(1.1*max([CCOf;CO2f;CCO2f],[],'all'),1)])
legend('Fitted A inlet','Fitted B_2 inlet')
xlabel('t/s')
ylabel('C/(mol/m^3)')
saveas(gcf,'Inletf.png')
hold off
figure()
plot(ts2,CCOf(:,n),'LineWidth',1.5)
hold on
plot(ts2,CO2f(:,n),'LineWidth',1.5)
plot(ts2,CCO2f(:,n),'LineWidth',1.5)
lgd=legend('Fitted A outlet','Fitted B_2 outlet','Fitted AB outlet');
ylim([0 3.5])
xlabel('t/s','FontSize',15)
ylabel('C/(mol/m^3)','FontSize',15)
lgd.FontSize=15;
ax = gca;
ax.FontSize=15;
saveas(gcf,'Outletf.png')
hold off
save('kfitPRBSfitdiP.mat')